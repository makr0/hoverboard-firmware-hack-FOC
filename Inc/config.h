...
#define BAT_LVL3                (360 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Yellow blink: no beep 
...
#define BAT_LVL1                (340 * BAT_CELLS * BAT_CALIB_ADC) / BAT_CALIB_REAL_VOLTAGE    // Red blink:    fast beep. Your battery is almost empty. Charge now! [V*100/cell]. In this case 3.50 V/cell
...
#define DEFAULT_RATE                20000 // 480   // 30.0f [-] lower value == slower rate [0, 32767] = [0.0, 2047.9375]. Do NOT make rate negative (>32767)
#define DEFAULT_FILTER              65535  // Default for FILTER 0.1f [-] lower value == softer filter [0, 65535] = [0.0 - 1.0].
...
*/
  ...
  #define ADC2_MID            1800      // mid ADC2-value while poti at minimum-position (ADC2_MIN - ADC2_MAX)
  #define ADC2_MAX            3100      // max ADC2-value while poti at maximum-position (0 - 4095)
  #define CMD1_DEADBAND 10            // steer
  #define CMD2_DEADBAND 10           // throttle
  //  If VAL_floatingPoint >= 0, VAL_fixedPoint = VAL_floatingPoint * 2^14
  // If VAL_floatingPoint < 0,  VAL_fixedPoint = 2^16 + floor(VAL_floatingPoint * 2^14).

  #define SPEED_COEFFICIENT   16383    // 1.0f -  0.0 to ~2.0?
  #define STEER_COEFFICIENT   49152    // -1.0f -  -1.0 to 1.0
  #define STEER_TANK // TANK style steering: ADC1 -> speed-L ADC2 -> speed-R
...
// ################################# VARIANT_NUNCHUK SETTINGS ############################
  ...
  */
  ...
  #define STEER_COEFFICIENT  8192 // 0.5f // 62259   // -0.2f
#endif
...
#define MULTIPLE_TAP_NR       0 //2 * 2      // [-] Define tap number: MULTIPLE_TAP_NR = number_of_taps * 2, number_of_taps = 1 (for single taping), 2 (for double tapping), 3 (for triple tapping), etc...
